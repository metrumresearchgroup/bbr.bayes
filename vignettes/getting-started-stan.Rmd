...
When modeling with Stan, `bbr.bayes` does _not_ use [`bbi`](https://metrumresearchgroup.github.io/bbr/articles/getting-started.html#installing-bbi), so you will not need it installed. However, you _will_ need `cmdstan` installed. Please see [the cmdstanr docs](http://mc-stan.org/cmdstanr/articles/cmdstanr.html#Installing_CmdStan) for instructions.

# Initial modeling run

## Create model object

To begin modeling, first create a model object using [`new_model()`](https://metrumresearchgroup.github.io/bbr/reference/new_model.html). This is an S3 object which you will pass to all of the other `bbr` and `bbr.bayes` functions that submit, summarize, or manipulate models.
...
The call above will look for a directory called `{MODEL_DIR}/mod1`. _Inside_ this directory it will expect to find the files `mod1.stan` and `mod1-standata.R`. These "required files" will be discussed below, but you can find even more detail in `?bbr_stan`. 
...
This `new_model()` call will also create a `mod1.yaml` file that will sit next to this directory on disk. This YAML file  stores model metadata like the description and tags (discussed below). For now the YAML file will look very bare, but hopefully its purpose will become more clear later.
...
For Stan models, model submission wraps the [cmdstanr::sample()](https://mc-stan.org/cmdstanr/reference/model-method-sample.html) method. 
...
To submit a model to be run, you simply pass the model object to [`submit_model()`](https://metrumresearchgroup.github.io/bbr.bayes/reference/stan_submit_model.html).
...
It is common to run a model, make some tweaks to it, and then run it again. However, to avoid accidentally deleting model outputs, `bbr` will error by default if it sees existing output when trying to submit a model. To automatically overwrite any previous model output, just pass `.overwrite = TRUE` to [`submit_model()`](https://metrumresearchgroup.github.io/bbr.bayes/reference/stan_submit_model.html).


## Summarize model

The [`submit_model()`](https://metrumresearchgroup.github.io/bbr.bayes/reference/stan_submit_model.html) call will return the fit model object from `cmdstanr`. You can read all about this object in the [`CmdStanMCMC` object docs](https://mc-stan.org/cmdstanr/reference/CmdStanMCMC.html). It has several methods for diagnostics and model summaries.
...
The [`submit_model()`](https://metrumresearchgroup.github.io/bbr.bayes/reference/stan_submit_model.html) call automatically saves this fit object to disk as well. You can pass either a path or model object to `bbr.bayes::read_fit_model()` and it will re-load the `CmdStanMCMC` object into memory.
...
```{r}
log_df %>%
  dplyr::select(run, num_divergent, dplyr::starts_with("emax")) %>% 
  knitr::kable()
```
