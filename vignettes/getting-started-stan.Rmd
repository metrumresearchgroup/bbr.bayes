 ...
 * `<run>-stanargs.R` - Contains a named list with all of the
   arguments that will be passed through to the [`$sample()`][sample]
   method of `?cmdstanr::CmdStanModel`.

 * `<run>-standata.R` - Contains all necessary R code to read in any
   source data and transform them to a Stan-ready data object (list).

 * `<run>-init.R` - This file contains all necessary R code to create
   the initial values passed to the cmdstanr's [`$sample()`][sample]
   method.
